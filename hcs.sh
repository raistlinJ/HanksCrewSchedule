...
# Enable bundled services unless the user has pointed HanksCrewSchedule at external
...
cmd_setup() {
  ...
  echo "  3) local    — No proxy, plain http on localhost (for trying HanksCrewSchedule out)"
  echo ""
  ...
  {
    cat <<ENVEOF
# HanksCrewSchedule Self-Hosted Configuration
...
cmd_start() {
  ...
  ensure_unlimited_seats
  echo ""
  echo "HanksCrewSchedule is starting at ${NEXT_PUBLIC_BASE_URL:-https://${DOMAIN:-localhost}}"
...
cmd_stop() {
  check_docker
  docker compose down
  echo "HanksCrewSchedule has been stopped."
}
...
cmd_restart() {
  ...
  echo "HanksCrewSchedule has been restarted."
}
...
fail_upgrade() {
  ...
  info "Run './rallly.sh start' to start HanksCrewSchedule on PostgreSQL $old_major as before."
  exit 1
}
...
cmd_upgrade_db() {
  ...
  echo ""
  echo "    1. Stop HanksCrewSchedule (it stays offline until the upgrade finishes)"
  ...
  echo "    4. Point .env at the new volume and restart HanksCrewSchedule"
  echo ""
  ...
  echo ""
  info "Stopping HanksCrewSchedule..."
  docker compose down
  ...
  info "Restarting HanksCrewSchedule..."
  docker compose up -d
