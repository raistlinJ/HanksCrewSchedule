...
import { PollResultsPage } from "@/features/poll/components/poll-results-page";
import { loadPublicPollResults } from "@/features/poll/loaders";
...
export default async function Page(props: {
  ...
  await loadPublicPollResults(urlId);
      ...
      <UserProvider user={session?.user ?? null}>
        ...
        >
          <PermissionProvider impersonatedUserId={impersonatedUserId}>
            <Providers>
              <div className="page-bg-gray-100 min-h-dvh overflow-auto dark:bg-gray-900">
                <PollResultsPage publicView />
              </div>
            </Providers>
          </PermissionProvider>
        </DeviceDateTimeProvider>
...
export async function generateMetadata(props: {
  ...
  const poll = await loadPublicPollResults((await props.params).urlId);

  return { title: `Results – ${poll.title}` };
}
