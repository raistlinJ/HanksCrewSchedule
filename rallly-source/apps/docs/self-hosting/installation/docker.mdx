...
After the stack is running, the user whose email matches `INITIAL_ADMIN_EMAIL` can navigate to `/control-panel` and will automatically receive the admin role. See the [Control Panel guide](/self-hosting/control-panel) for details.
