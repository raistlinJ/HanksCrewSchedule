...
<ParamField path="INITIAL_ADMIN_EMAIL">
  The user who signs in with this email is automatically granted the admin role when they visit `/control-panel`. See the [Control Panel guide](/self-hosting/control-panel).
</ParamField>
...
<ParamField path="EMAIL_DELIVERY_DISABLED" default="false">
  Set to `true` to suppress all outbound email delivery while testing. Email
  flows still run, but neither SMTP nor SES is contacted.
</ParamField>
